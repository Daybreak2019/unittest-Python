...
class TestLoader(object):
    ...
    def isExcCale (self, testCaseClass):
        # wen, for execute single case
        curCase =  os.environ.get ("case_name")
        if curCase is None:
            return True
        else:
            _, caseClass = curCase.split ("#")
            #print (caseClass, " ------------- ", testCaseClass, " => ", caseClass.find (testCaseClass))
            if caseClass.find (testCaseClass) == -1:
                return False
            else:
                return True

    def loadTestsFromTestCase(self, testCaseClass):
        if self.isExcCale (testCaseClass.__name__) == False:
            testCaseNames = []
